document.getElementById('brute-force-button').addEventListener('click', function() {
    const img = new Image();
    img.src = document.getElementById('file-input').files[0];
    img.onload = function() {
        // Perform brute-force layer separation
        const bruteForceLayers = bruteForceLayerSeparation(img);
        displayBruteForceLayers(bruteForceLayers);
    };
});

function bruteForceLayerSeparation(img) {
...
function displayBruteForceLayers(layers) {
