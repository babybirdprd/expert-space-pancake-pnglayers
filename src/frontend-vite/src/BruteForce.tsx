...
const BruteForce: React.FC<BruteForceProps> = ({ imageUrl, onStart }) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);

  const startBruteForce = (img: HTMLImageElement) => {
    ...
    const bruteForceLayers = layers.map(layer => {
  ...
  return (
    <div>
      <h2>Brute Force</h2>
      <img src={imageUrl} alt="Brute Force" onLoad={(e) => startBruteForce(e.target as HTMLImageElement)} />
